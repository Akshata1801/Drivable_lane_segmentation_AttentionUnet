...
preprocess = transforms.Compose([
                    transforms.ToTensor(),
                    transforms.Resize(size=(160,320)),
...
class BDD100k_dataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        label = self.labels[index]
        transform = transforms.Resize(size=(180,360))
        # label_resize = label.reshape(180,360)
        label_image  = torch.from_numpy(label).long()
        label_image = label_image.resize_(160,320)
        # label_resize = transform(label)
...
def train_validate_model(model, num_epochs, model_name, criterion, optimizer, 
    ...
    for epoch in range(num_epochs):
        ...
        if validation_loss <= min_val_loss:
            ...
            model_name = "model_"+str(min_val_loss)
...
def predict_video(model, model_name, input_video_path, output_dir, 
    ...
    while(input_handle.isOpened()):
        ...
        if ret == True:
            ...
            # print("pt_image shape : "+str(pt_image.shape))
            ...
            # print("cm_labels shape : "+str(cm_labels.shape))
            cm_labels = cv2.resize(cm_labels.astype(np.uint8), (frame.shape[1], frame.shape[0]), interpolation=cv2.INTER_NEAREST)
            cv2_imshow(cm_labels)
